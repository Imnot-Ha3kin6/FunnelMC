...
public class Xbox {
	...
	private final JsonParser jsonParser = FunnelMC.instance.fileManagement.jsonParser;
	...
	public String getUserToken(ECPublicKey publicKey, ECPrivateKey privateKey) throws Exception {
		...
		String responce = FunnelMC.instance.fileManagement.getTextFromInputStream(connection.getInputStream());
	...
	public String requestMinecraftChain(String xsts, ECPublicKey publicKey) throws Exception {
		...
		connection.setRequestProperty("Client-Version", Client.instance.bedrockCodec.getMinecraftVersion());
	...
	private void writeJsonObjectToPost(HttpsURLConnection connection, JsonObject jsonObject) throws Exception {
		...
		dataOutputStream.writeBytes(FunnelMC.instance.fileManagement.normalGson.toJson(jsonObject));
	...
	private void addSignatureHeader(HttpsURLConnection httpsURLConnection, JsonObject postData, ECPrivateKey privateKey) throws Exception {
		...
		bytesToSign.write(new byte[] { 0 });
		bytesToSign.write(FunnelMC.instance.fileManagement.normalGson.toJson(postData).getBytes());
		bytesToSign.write(new byte[] { 0 });
